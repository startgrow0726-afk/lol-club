...
// Firebase는 빈 배열을 삭제하므로, 불러온 데이터를 정규화
const fixTeams=data=>{
...
export default function App(){
  ...
  // Firebase 실시간 구독 - 다른 사람이 변경하면 자동 반영
  useEffect(()=>{
    const unsub1 = subscribeData("teams", (data) => {
      setTeams(fixTeams(data));
      setLoaded(true);
    });
    const unsub2 = subscribeData("schedules", (data) => {
      setSched(fixSched(data));
    });
    const unsub3 = subscribeData("matches", (data) => {
      setMatches(fixMatches(data));
    });
    return () => { unsub1(); unsub2(); unsub3(); };
  },[]);

  // Firebase에 저장
  const persist = useCallback((t,s,m) => {
    saveData("teams", t);
    saveData("schedules", s);
    saveData("matches", m);
  },[]);
    ...
    <div style={S.root}>
      ...
      <main style={{...S.main,animation:anim?"fi .3s ease":"fo .14s ease"}}>
        ...
        {tab==="teams"&&isAdmin&&<div>
          <div style={S.sh}><h2 style={S.st}>👥 인원 · 팀 관리</h2><p style={S.sd}>이름·닉네임 클릭 수정 / 드롭다운으로 팀 이동</p></div>
        ...
        {tab==="schedule"&&isAdmin&&<div>
          ...
          {renderBest()}
          <h3 style={{...S.sub,marginTop:28}}>📋 전체 캘린더</h3>
          {renderCal(false)}
        ...
        <div style={{...S.mod,maxWidth:640}} onClick={e=>e.stopPropagation()}>
          ...
          <div style={{display:"flex",gap:12,flexWrap:"wrap"}}>
            <div style={{flex:1,minWidth:200}}>
              <div style={{fontSize:13,fontWeight:700,color:"#5383e8",marginBottom:8}}>🔷 블루팀</div>
              {POSITIONS.map((pos,idx)=>{const p=mBlue[idx];const active=pickSide?.side==="blue"&&pickSide?.idx===idx;return<div key={idx} style={{...S.pickSlot,...(active?{borderColor:"#5383e8",background:"rgba(83,131,232,.08)"}:{})}}><span style={{color:PC[pos],fontSize:12,minWidth:50}}>{PI[pos]} {pos}</span>{p?<div style={{display:"flex",alignItems:"center",gap:4,flex:1}}><span style={{fontSize:12,color:"#f0e6d2"}}>{p.name}</span><button onClick={()=>removeSlot("blue",idx)} style={{...S.noB,fontSize:9,padding:"1px 4px"}}>✕</button></div>:<button onClick={()=>setPickSide({side:"blue",idx})} style={{...S.pickBtn,...(active?{borderColor:"#5383e8",color:"#5383e8"}:{})}}>선택</button>}</div>})}
            </div>
            <div style={{flex:1,minWidth:200}}>
              <div style={{fontSize:13,fontWeight:700,color:"#e84057",marginBottom:8}}>🔶 레드팀</div>
              {POSITIONS.map((pos,idx)=>{const p=mRed[idx];const active=pickSide?.side==="red"&&pickSide?.idx===idx;return<div key={idx} style={{...S.pickSlot,...(active?{borderColor:"#e84057",background:"rgba(232,64,87,.08)"}:{})}}><span style={{color:PC[pos],fontSize:12,minWidth:50}}>{PI[pos]} {pos}</span>{p?<div style={{display:"flex",alignItems:"center",gap:4,flex:1}}><span style={{fontSize:12,color:"#f0e6d2"}}>{p.name}</span><button onClick={()=>removeSlot("red",idx)} style={{...S.noB,fontSize:9,padding:"1px 4px"}}>✕</button></div>:<button onClick={()=>setPickSide({side:"red",idx})} style={{...S.pickBtn,...(active?{borderColor:"#e84057",color:"#e84057"}:{})}}>선택</button>}</div>})}
            </div>
          </div>
...
const S={
  ...
  loginBtn:{background:"linear-gradient(180deg,#c8aa6e,#785a28)",border:"none",borderRadius:8,padding:"12px",color:"#010a13",fontSize:15,fontWeight:700,fontFamily:"'Noto Sans KR'",boxShadow:"0 4px 16px rgba(200,170,110,.3)"},
